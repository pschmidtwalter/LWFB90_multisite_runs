...
directly on the output of simulation, and can be used to do some custom
...
`NULL`, because `write.csv` has no return value when a file is specified.
However, we can see that the simulation results were written to our
'output'-directory:
```{r}
list.files("output")
```
Our output function did not use any external arguments, all information (data
and names of the input to create a file name) was internally generated within
`run_multisite_LWFB90()`. Similarily, we could have written the simulation
results to a database. To do so, we just needed to pass a database connection
object as argument to `output_fun`, that could simply be specified as argument
(via the `...` place-holder) in the call of `run_multisite_LWFB90()`. We will
...
period. To reduce code of the function, we outsource some of the code in
functions that can be found in the directory 'R-Functions'. We source the
functions:
...
```
The first function (`aggr_swati`) aggregates the daily layer-wise soil moisture
and fluxes variables (`layer_output`-data.frame) to daily values, while the
other functions aggregate daily values to monthly or vegetation period
representations, for which the beginning and end day-of-year of the vegetation
period have to be specified as vectors covering the years of the simulation. The
aggregation of the water fluxes simply take the output object `daily_output`,
while soil moisture state aggregation functions are designed to use the returned
daily values from the first function `aggr_swati`. Referring to these functions,
we can define our `output_fun` to combine the results in two tables, and write
them to two different tables (>70 columns!) in a database:

```{r}
outfun_nfiwads <- function(x, clim_nm, soil_nm, param_nm, db_out) {
  ...
  out_vegper <- data.table(site_id = clim_nm, parset = param_nm,
  ...
  setnames(out_vegper, names(out_vegper), tolower(names(out_vegper)))

  # write to db
  if (dbExistsTable(db_out, "nfiwads_monthly")) {
    chk_m <-dbAppendTable(db_out, "nfiwads_monthly", out_monthly)
  } else {
    dbCreateTable(db_out, "nfiwads_monthly", out_monthly)
    chk_m <-dbAppendTable(db_out, "nfiwads_monthly", out_monthly)
  }
  
  if (dbExistsTable(db_out, "nfiwads_vegper")) {
    chk_vp <- dbAppendTable(db_out, "nfiwads_vegper", out_vegper)
  } else {
    dbCreateTable(db_out, "nfiwads_vegper", out_vegper)
    chk_vp <- dbAppendTable(db_out, "nfiwads_vegper", out_vegper)
  }
...
The `db_out` database connection needs to be created first. Like the input
...
And see that out database now contains two tables, populated with the test-data:

```{r}
dbListTables(db_out)
dbGetQuery(db_out, "select * from nfiwads_monthly limit 6")
#remove test-tables

...
test3[1]
```
Because we passed `rtrn_output = FALSE` to `run_LWFB90()`, the entries of the returned list do not contain any simulation results. As well, the entry `output_fun` is `NULL`, because `write.csv` has no return value when a file is specified. Additional arguments can be also passed to `output_fun`, when specified in the call of `run_multisite_LWFB90()`. These could be database connection objects or other arguments accepted by a custom `output_fun`.
